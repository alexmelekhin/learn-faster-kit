...
def sync_concept_file(topic_dir: Path, item: dict) -> None:
    """Mirror review schedule concepts for quiz selection compatibility."""
    concepts_dir = topic_dir / "concepts"
    concepts_dir.mkdir(exist_ok=True)
    slug = concept_slug(item["concept"])
    concept_path = concepts_dir / f"{slug}.json"
    ...
    try:
        ...
        tell application "Reminders"
            tell list "Learn FASTER"
                make new reminder with properties {{name:"Review: {concept} ({topic_slug})", due date:date "{reminder_date}", body:"Time to review '{concept}' from your {topic_slug} learning. Run $learn-faster in Codex or /review in Claude Code."}}
            end tell
        end tell
        '''
...
def add_review_item(topic_slug: str, concept: str, base_dir: str = ".learning"):
    ...
    sync_concept_file(topic_dir, review_item)
    ...
    output = {
        ...
        "llm_directive": "Concept added to review schedule. Ask the user directly whether they want to continue learning, practice, or stop for today.",
...
def mark_reviewed(topic_slug: str, concept: str, base_dir: str = ".learning"):
    ...
    for item in schedule["reviews"]:
        if item["concept"].lower() == concept.lower():
            ...
            sync_concept_file(topic_dir, item)
