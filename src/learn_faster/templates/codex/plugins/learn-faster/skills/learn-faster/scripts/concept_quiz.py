...
def concept_slug(concept: str) -> str:
    """Create a portable filename slug for concept metadata."""
...
def get_least_asked_concepts(topic_slug: str, limit: int = 3, base_dir: str = ".learning"):
        ...
        for concept_file in concepts_dir.glob("*.json"):
            with open(concept_file, "r") as f:
                ...
                concepts.append({
                    ...
                    "slug": data.get("concept_slug", concept_slug(data["concept"])),
    ...
    if not concepts:
        ...
        if schedule_path.exists():
            ...
            for item in schedule.get("reviews", []):
                concepts.append({
                    "concept": item["concept"],
                    "slug": concept_slug(item["concept"]),
                    "review_count": item.get("review_count", 0),
                    "last_reviewed": item.get("last_reviewed"),
                    "learned_date": item.get("learned_date")
                })

    # Sort by review_count (ascending) then by learned_date (oldest first)
    concepts.sort(key=lambda x: (x["review_count"], x["learned_date"] or ""))
...
def generate_quiz_directive(topic_slug: str, base_dir: str = ".learning"):
    ...
    directive = f"""
After logging progress, ask the user a quick conceptual quiz question directly.
...
After quiz, record the attempt with:
python3 .learning/scripts/concept_quiz.py record {topic_slug} "<concept>" <true|false>
"""
...
def record_quiz_attempt(topic_slug: str, concept: str, correct: bool, base_dir: str = ".learning"):
    ...
    concept_file = concepts_dir / f"{slug}.json"
    ...
    if not concept_file.exists():
        ...
        with open(concept_file, "w") as f:
            json.dump({
                "concept": schedule_item["concept"],
                "concept_slug": slug,
                "review_count": schedule_item.get("review_count", 0),
                "last_reviewed": schedule_item.get("last_reviewed"),
                "learned_date": schedule_item.get("learned_date")
            }, f, indent=2)
    ...
    if schedule_path.exists():
        ...
        for item in schedule.get("reviews", []):
            if item["concept"].lower() == concept.lower():
                item["quiz_history"] = data["quiz_history"]
                item["quiz_count"] = data["quiz_count"]
                item["quiz_correct_count"] = data["quiz_correct_count"]
                break
